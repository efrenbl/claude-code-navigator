...
Example:
    Command line usage:
        $ codemap watch /path/to/project -o .codemap.json

    Python API usage:
        >>> watcher = CodeMapWatcher('/path/to/project', '.codemap.json')
...
class CodeMapWatcher:
    ...
    Attributes:
        ...
        output_path: Path to the output .codemap.json file.
    ...
    Example:
        >>> watcher = CodeMapWatcher('/my/project', '.codemap.json')
        >>> watcher.start()
    """
    ...
    def __init__(
        ...
        Args:
            root_path: Path to the codebase root.
            output_path: Path to output file (default: .codemap.json).
...
def run_watch(args) -> None:
    ...
    watcher = CodeMapWatcher(
        root_path=args.path,
        output_path=getattr(args, "output", ".codemap.json"),
