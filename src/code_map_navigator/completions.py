#!/usr/bin/env python3
"""Shell completion scripts generator for Claude Code Navigator.

Generates bash and zsh completion scripts that provide autocompletion
for codemap commands and symbol names from the code map.
...
BASH_COMPLETION_TEMPLATE = """# Bash completion for codemap
# Generated by claude-code-navigator

_codemap_completions() {
            ...
            case "${prev}" in
                ...
                *)
                    if [[ "${cur}" == -* ]]; then
                        ...
                        # Try to complete symbol names from codemap
                        _codemap_complete_symbols
                    fi
                    ;;
            esac
            ;;
...
_codemap_complete_symbols() {
    # Try to read symbols from .codemap.json in current directory
    local mapfile=".codemap.json"
    if [[ -f "${mapfile}" ]]; then
...
complete -F _codemap_completions codemap
"""

ZSH_COMPLETION_TEMPLATE = """#compdef codemap
# Zsh completion for codemap
# Generated by claude-code-navigator
...
_codemap() {
    ...
    case "$state" in
        command)
            _describe -t commands 'codemap command' commands
            ;;
        args)
            case "$words[1]" in
                map)
                ...
                search)
                    _arguments \\
                        ...
                        '--deps[Show dependencies of a symbol]:symbol:_codemap_symbols' \\
                        '--stats[Show codebase statistics]' \\
