...
- `sample_codemap.json` - Example output from code_mapper.py
...
cd examples
code-map . -o demo-map.json --pretty
```
...
See `sample_codemap.json` for the JSON structure:
...
if [ ! -f "$PROJECT_PATH/.codemap.json" ]; then
    echo "Generating code map..."
    code-map "$PROJECT_PATH"
fi

# Search
echo "Searching for: $SEARCH_TERM"
RESULTS=$(code-search "$SEARCH_TERM" -m "$PROJECT_PATH/.codemap.json")
