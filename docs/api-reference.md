...
The `CodeMapper` class scans a codebase and generates a searchable index.
...
```python
CodeMapper(root_path: str, ignore_patterns: List[str] = None)
```
...
mapper = CodeMapper('/path/to/project', ignore_patterns=['*.test.py', 'vendor/'])
```
...
**Parameters:**
- `map_path` (str): Path to the .codemap.json file
...
```bash
code-map PATH [-o OUTPUT] [-i IGNORE...] [--pretty] [-v]
```

**Options:**
- `PATH`: Root directory to scan
- `-o, --output`: Output file (default: .codemap.json)
...
**Options:**
- `QUERY`: Search query
- `-m, --map`: Code map file (default: .codemap.json)
